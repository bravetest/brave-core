...
void BraveBrowserView::UpdateDevToolsForContents(
  ...
  if (split_view_) {
    ...
    split_view_->secondary_devtools_web_view()->SetWebContents(nullptr);
  }

  BrowserView::UpdateDevToolsForContents(web_contents,
                                         update_devtools_web_contents);
  if (split_view_ && split_view_->secondary_contents_web_view()->GetVisible()) {
    split_view_->UpdateSecondaryDevtoolsLayoutAndVisibility();
  }
