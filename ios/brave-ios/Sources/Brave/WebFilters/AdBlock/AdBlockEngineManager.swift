...
@MainActor class AdBlockEngineManager {
  ...
  /// The blocklist type this manager represents if we are grouping content blockers
  ...
  private func compileAvailableIfNeeded(
    ...
    do {
      ...
      guard self.checkNeedsCompile(for: compilableFiles) else {
        ...
        // We will also check this again under `ensureContentBlockers(...)`
        // but this is an optimization so that we don't call `combineRules` unecessarily.
